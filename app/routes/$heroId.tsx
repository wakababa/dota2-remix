    ...
    <div style={{ padding: 5 }}>
      ...
      <div style={{ display: "flex", flexDirection: "row", alignItems: "center", gap: 5 }}>
        <Col>
          ...
          {
            <video id={name} key={name} width="640" height="480" autoPlay playsinline muted loop>
              <source
                src={`https://cdn.cloudflare.steamstatic.com/apps/dota2/videos/dota_react/abilities/${heroName}/${name}.webm`}
                type="video/webm" />
            </video>
          }
        </Col>
      ...
      <Col overrideStyled={{ alignContent: "center", justifyContent: "center" }}>
        ...
        <Row>
          {abilities.map((ability: IAbility, key) => {
            ...
            return (
              <div key={key} onClick={() => setSelectedAbility(ability)}
                   style={{ display: "flex", flexDirection: "column", minWidth: 150, margin: 5 }}>
...
const ChangeHero = ({heroName}:{heroName:String}) => {
  const { heroId } = useParams();

  return (
          <div style={{ display: "flex", flexDirection: "row", justifyContent: "space-between" }}>
      <a style={{ fontSize: 25, textDecoration: "none", color: "white" }} href={`/`}>Home</a>
      <a style={{ fontSize: 25, textDecoration: "none", color: "white" }}>{heroName}</a>
      <Row >
        <a style={{ fontSize: 25, textDecoration: "none", color: "white",marginRight:20 }} href={`${Number(heroId) - 1}`}>Back</a>
